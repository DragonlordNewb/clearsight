...
#    node v is the length of the shortest path discovered so far between the node v and the starting 
#    node. Since initially no path is known to any other vertex than the source itself (which is 
#    a path of length zero), all other tentative distances are initially set to infinity. Set the 
...
#    one currently assigned to the neighbor and assign it the smaller one. For example, if the 
#    current node A is marked with a distance of 6, and the edge connecting it with a neighbor B 
#    has length 2, then the distance to B through A will be 6 + 2 = 8. If B was previously marked 
...
#    checked again (this is valid and optimal in connection with the behavior in step 6.: that 
#    the next nodes to visit will always be in the order of 'smallest distance from initial node 
#    first' so any visits after would have a greater distance).
